...
function renderCustomers(){
  ...
  $('#customersPage').innerHTML=`<div class="section-head"><div><h3>Customers / Pharmacies</h3><div class="muted">${rows.length} accessible customer(s)</div></div>${can('admin','salesman','erp_manager')?'<button id="addCustomerBtn" class="btn primary">+ Add Customer</button>':''}</div><div class="table-wrap"><table class="table"><thead><tr><th>Code</th><th>Customer / Pharmacy</th><th>Contact</th><th>Phone</th><th>Territory</th><th>Salesman</th><th>Credit Limit</th><th>Days</th><th>Free Scheme</th><th>Balance</th><th>Status</th><th></th></tr></thead><tbody>${rows.map(c=>`<tr><td>${esc(c.customerCode||'—')}</td><td><b>${esc(c.shopName||c.customerName||'')}</b><div class="metric-note">${esc(c.customerName||'')}</div></td><td>${esc(c.ownerName||'')}</td><td>${esc(c.phone||'')}</td><td>${esc(c.territory||c.routeArea||'')}</td><td>${esc(profileName(c.assignedSalesmanId))}</td><td>${money(c.creditLimit)}</td><td>${fmt(c.creditDays)}</td><td>${c.complimentarySchemeEligible===true?'<span class="badge status-approved">Eligible</span>':'<span class="badge">Not Eligible</span>'}</td><td><b>${money(Number(c.currentBalance||0))}</b></td><td><span class="badge status-${c.active===false?'inactive':'active'}">${c.active===false?'Inactive':'Active'}</span></td><td><div class="actions">${(canManageCustomers()||c.assignedSalesmanId===state.user.uid)?`<button class="btn ghost small" data-customer="${c.id}">Edit</button>`:''}${canDeactivateCustomer()&&c.active!==false?`<button class="btn danger small" data-deactivate-customer="${c.id}">Deactivate</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="12" class="empty">No customers yet.</td></tr>'}</tbody></table></div>`;
...
function openCustomerForm(id=null){
  ...
  $('#customerForm').onsubmit=async e=>{
    ...
    try{
      if(existing){
        const data=can('salesman')
          ? {territory:$('#cTerritory').value.trim(),routeArea:$('#cTerritory').value.trim(),address:$('#cAddress').value.trim(),creditLimit:Number(existing.creditLimit||0),creditDays:Number(existing.creditDays||0),updatedAt:serverTimestamp()}
          : {customerName:$('#cName').value.trim(),shopName:$('#cShop').value.trim(),ownerName:$('#cName').value.trim(),phone:$('#cPhone').value.trim(),territory:$('#cTerritory').value.trim(),routeArea:$('#cTerritory').value.trim(),assignedSalesmanId,creditLimit:Number($('#cCreditLimit').value||0),creditDays:Number($('#cCreditDays').value||0),ntnCnic:$('#cTaxId').value.trim(),complimentarySchemeEligible:$('#cSchemeEligible').value==='true',address:$('#cAddress').value.trim(),updatedAt:serverTimestamp()};
        await updateDoc(doc(db,'customers',existing.id),data);
        await writeAudit('customer_updated','customer',existing.id,{customerCode:existing.customerCode||'',fields:can('salesman')?['territory','address']:['profile','complimentarySchemeEligible']});
        closeModal(); toast('Customer updated.'); return;
      }
      ...
      const duplicate=state.customers.find(x=>
        (customerName && normalizeKey(x.customerName||x.ownerName)===normalizeKey(customerName)) ||
        (shopName && normalizeKey(x.shopName)===normalizeKey(shopName)) ||
        (phone && normalizeKey(x.phone)===normalizeKey(phone))
      );
      if(duplicate) throw new Error(`Possible duplicate customer: ${duplicate.customerCode||''} ${duplicate.shopName||duplicate.customerName||''}`);
      ...
      await runTransaction(db,async tx=>{
        ...
        tx.set(counterRef,{next:next+1},{merge:true});
        tx.set(customerRef,{customerCode,customerName,shopName,ownerName:customerName,phone,territory:$('#cTerritory').value.trim(),routeArea:$('#cTerritory').value.trim(),assignedSalesmanId,creditLimit:Number($('#cCreditLimit').value||0),creditDays:Number($('#cCreditDays').value||0),ntnCnic:$('#cTaxId').value.trim(),complimentarySchemeEligible:canAuthorizeComplimentary()&&$('#cSchemeEligible').value==='true',openingBalance,currentBalance:openingBalance,address:$('#cAddress').value.trim(),active:true,createdBy:state.user.uid,createdAt:now,updatedAt:now});
        ...
        auditSet(tx,'customer_created','customer',customerRef.id,{customerCode,shopName,openingBalance,complimentarySchemeEligible:canAuthorizeComplimentary()&&$('#cSchemeEligible').value==='true'});
      });
...
function openLedgerLinkedDocument(customerId,entryId){
  const entry=ledgerRowsForCustomer(customerId).find(r=>String(r.id)===String(entryId)); if(!entry)return;
  const payment=entry.sourceType==='payment'?state.payments.find(p=>p.id===entry.sourceId):null;
  showModal(`Ledger Document — ${entry.documentNo||entry.invoiceNo||entry.entryType}`,`<div class="card"><div class="two-col"><div><b>Type:</b> ${esc(String(entry.entryType||'').replaceAll('_',' '))}<br><b>Date:</b> ${date(entry.transactionDate)}<br><b>Document:</b> ${esc(entry.documentNo||'')}<br><b>Invoice:</b> ${esc(entry.invoiceNo||'—')}</div><div><b>Debit:</b> ${money(entry.debit||0)}<br><b>Credit:</b> ${money(entry.credit||0)}<br><b>Paid Units:</b> ${fmt(entry.paidUnits||0)}<br><b>Free Units:</b> ${fmt(entry.complimentaryUnits||0)}</div></div>${payment?`<hr><p><b>Payment Method:</b> ${esc(payment.method||'')}<br><b>Reference:</b> ${esc(payment.reference||'')}<br><b>Entered By:</b> ${esc(profileName(payment.enteredBy))}</p>`:''}<p><b>Notes:</b> ${esc(entry.notes||'')}</p><div class="metric-note">Linked document ID: ${esc(entry.documentId||entry.sourceId||entry.id||'')}</div></div>`);
}

function printCustomerStatement(customer,rows,balance){
  const body=`<h2>Naturegen Distribution — Customer Statement</h2><p><b>${esc(customer.customerCode||'')}</b> ${esc(customer.shopName||customer.customerName||'')}</p><p>${esc(customer.address||'')} ${esc(customer.phone||'')}</p><table><thead><tr><th>Date</th><th>Type</th><th>Document</th><th>Paid Units</th><th>Free Units</th><th>Delivered</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${date(r.transactionDate)}</td><td>${esc(r.entryType)}</td><td>${esc(r.documentNo||'')}</td><td>${fmt(r.paidUnits||0)}</td><td>${fmt(r.complimentaryUnits||0)}</td><td>${fmt(r.totalDeliveredUnits||0)}</td><td>${money(r.debit||0)}</td><td>${money(r.credit||0)}</td><td>${money(r.running)}</td></tr>`).join('')}</tbody></table><h3>Closing Balance: ${money(balance)}</h3>`; const w=window.open('','_blank','width=1000,height=760');if(!w)return toast('Allow popups to print statements.',true);w.document.write(`<html><head><title>Customer Statement</title><style>body{font-family:Arial;padding:28px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #bbb;padding:7px}</style></head><body>${body}<script>window.onload=()=>window.print()<\/script></body></html>`);w.document.close();
}
function downloadLedgerCsv(customer,rows){const data=[['Date','Type','Document','Invoice','Paid Units','Complimentary Units','Total Delivered','Debit','Credit','Running Balance','Due','Notes'],...rows.map(r=>[date(r.transactionDate),r.entryType,r.documentNo||'',r.invoiceNo||'',r.paidUnits||0,r.complimentaryUnits||0,r.totalDeliveredUnits||0,r.debit||0,r.credit||0,r.running,date(r.dueDate),r.notes||''])];const csv=data.map(row=>row.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\n');downloadBlob(csv,`${customer.customerCode||'customer'}-ledger-${todayISO()}.csv`,'text/csv');}
