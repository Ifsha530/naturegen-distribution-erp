  ...
  $('#customerForm').onsubmit=async e=>{
    ...
    try{
      ...
      await runTransaction(db,async tx=>{
        ...
        tx.set(counterRef,{next:next+1},{merge:true});
        tx.set(customerRef,{customerCode,customerName,shopName,ownerName:customerName,phone,territory:$('#cTerritory').value.trim(),routeArea:$('#cTerritory').value.trim(),assignedSalesmanId,creditLimit:Number($('#cCreditLimit').value||0),creditDays:Number($('#cCreditDays').value||0),ntnCnic:$('#cTaxId').value.trim(),complimentarySchemeEligible:management&&$('#cSchemeEligible').value==='true',openingBalance,currentBalance:openingBalance,address:$('#cAddress').value.trim(),active:true,createdBy:state.user.uid,createdAt:now,updatedAt:now});
        ...
        auditSet(tx,'customer_created','customer',customerRef.id,{customerCode,shopName,openingBalance,complimentarySchemeEligible:management&&$('#cSchemeEligible').value==='true'});
      });
